...
export default function VoiceCapture({ onPatientAdded }) {
  ...
  const getVitalCategory = (keyword) => {
    ...
    if (["spo2", "oxygen", "o2", "sat", "saturation"].includes(normalized))
      return "SpO2";
  ...
  const extractKeywords = (text) => {
    ...
    vitalKeywords.forEach((keyword) => {
      ...
      const valueMatch = cleaned.match(
        /^([\d]+(?:[./\-][\d]+)?(?:\s*(?:mmhg|bpm|%|°c|°f|celsius|fahrenheit|mg\/dl|kg|lbs|cm)?)?)/i,
      );
      ...
      const symptomPattern = symptomKeywords
        ...
        .join("|");
      value = value
        .replace(new RegExp(`\\s+(${symptomPattern}).*$`, "i"), "")
        .trim();
  ...
  const startRecording = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (finalTextChunk)
        finalTranscriptRef.current =
          `${finalTranscriptRef.current} ${finalTextChunk}`.trim();
    ...
    recognition.onerror = (event) => {
      ...
      const errorMap = {
        "not-allowed":
          "Microphone permission denied. Allow mic access in browser settings and retry.",
        "no-speech":
          "No speech detected. Please speak closer to the microphone and retry.",
        "audio-capture":
          "No microphone detected. Connect a microphone and try again.",
        network:
          "Network issue while using speech service. Check your connection and retry.",
      };
      setStatus({
        type: "error",
        message: errorMap[event.error] || `Microphone error: ${event.error}`,
      });
    };
    ...
    if (finalRaw) {
      ...
      try {
        ...
        if (refined) {
          ...
          setStatus({
            type: "success",
            message: 'Transcript refined. Review and click "Analyze & Triage".',
          });
        } else {
          setStatus(null);
        }
  ...
  const handleSubmit = async () => {
    ...
    if (!text) {
      setStatus({
        type: "error",
        message: "Please record or type patient information before submitting.",
      });
      return;
    }
    ...
    setStatus({
      type: "info",
      message: "🧠 AI is analyzing patient data and assigning priority...",
    });
    try {
      const patient = await submitPatient(text);
      const priorityLabel =
        { RED: "Critical", YELLOW: "Urgent", GREEN: "Standard" }[
          patient.priority
        ] || patient.priority;
      setStatus({
        type: "success",
        message: `Patient added successfully. Priority: ${priorityLabel}`,
      });
      setTranscript("");
      setInterimTranscript("");
      finalTranscriptRef.current = "";
      ...
      setStatus({
        type: "error",
        message: `Failed to process: ${err.message}. Is the backend running at localhost:8081?`,
      });
    } finally {
  ...
  const MicIcon = ({ size = 22 }) => (
    ...
    >
      ...
      />
      <path
        d="M6 10.5a6 6 0 0 0 12 0M12 16.5V20M9 20h6"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
  ...
  const AnalyzeIcon = ({ size = 18 }) => (
    ...
    >
      <path
        d="M8 4h8v4h4v8h-4v4H8v-4H4V8h4V4Z"
        fill="currentColor"
        opacity="0.2"
      />
      <path
  ...
  const StepListenIcon = ({ size = 34 }) => (
    ...
    >
      <path
        d="M12 3.8a2.8 2.8 0 0 0-2.8 2.8v5.3a2.8 2.8 0 0 0 5.6 0V6.6A2.8 2.8 0 0 0 12 3.8Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.8"
      />
      <path
        d="M6.2 11.2a5.8 5.8 0 0 0 11.6 0M12 17v2.8"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
      />
      <path
        d="M4.8 8.4a8.7 8.7 0 0 0 0 7.2M19.2 8.4a8.7 8.7 0 0 1 0 7.2"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.4"
        strokeLinecap="round"
  ...
  const StepExtractIcon = ({ size = 34 }) => (
    ...
    >
      <path
        d="M7.5 3.8h7.2l3.1 3.2v12.1a1.7 1.7 0 0 1-1.7 1.7H7.5a1.7 1.7 0 0 1-1.7-1.7V5.5a1.7 1.7 0 0 1 1.7-1.7Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.7"
        strokeLinejoin="round"
      />
      <path
        d="M14.7 3.8V7h3.1M9 10h6M9 13h6M9 16h3.8"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.7"
        strokeLinecap="round"
  ...
  const StepScoreIcon = ({ size = 34 }) => (
    ...
    >
      <path
        d="M12 3.8 5.8 6.2v4.9c0 4 2.4 7.7 6.2 9.1 3.8-1.4 6.2-5.1 6.2-9.1V6.2L12 3.8Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.7"
        strokeLinejoin="round"
      ...
      <div className="voice-panel-header-premium">
        <div className="voice-panel-header-content">
          ...
          <div className="voice-panel-heading-block">
            <h1 className="voice-panel-title-premium">
              Voice Intelligence for Emergency Care
            </h1>
            <p className="voice-panel-subtitle-premium">
              Real-time patient intake powered by clinical AI
            </p>
          </div>
        </div>
      ...
      <div className="voice-panel-main-premium">
        {!isSupported ? (
          <div className="speech-warning-premium">
            ...
            <span>
              Web Speech API not supported. Please type patient information
              manually.
            </span>
          </div>
        ) : (
              ...
              <div className="voice-record-card-premium">
                <div className="card-inner-premium">
                  <div className="record-header">
                    <span className="record-header-label">
                  ...
                  <div className="recording-pro-tip">
                    ...
                    <span className="pro-tip-text">
                      <strong>Pro Tip:</strong> Include age, key symptoms, pain
                      level, and duration for best AI triage accuracy.
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="voice-input-card-premium">
                  <div className="card-inner-premium">
                    <div className="input-header">
                      <span className="input-header-label">
                        <PromptIcon size={16} /> Patient Information
                      </span>
                    </div>

                    <textarea
                      id="transcript-input"
                      className={`transcript-box-premium${isRecording ? " listening" : ""}${isRefining ? " refining" : ""}`}
                      lang={language}
                      value={transcript}
                        ...
                        {extractedKeywords.symptoms.length > 0 && (
                          <div className="keyword-section">
                            ...
                            <div className="keyword-tags">
                              {extractedKeywords.symptoms.map(
                                (symptom, idx) => (
                                  <span
                                    key={idx}
                                    className="keyword-tag symptom-tag"
                                  >
                                    {symptom}
                                  </span>
                                ),
                          ...
                          <div className="keyword-section">
                            ...
                            <div className="keyword-tags">
                              {extractedKeywords.vitals.map((vital, idx) => (
                                <span
                                  key={idx}
                                  className="keyword-tag vital-tag"
                                >
                                  {vital}
                                </span>
                              ))}
                    ...
                    {status && (
                      <div
                        className={`status-message-premium status-${status.type}`}
                      >
                        <span
                          className={`status-icon-premium status-${status.type}-icon`}
                        >
                          {status.type === "error"
                            ? "✕"
                            : status.type === "success"
                    ...
                    <div className="input-actions-row">
                      <button
                        id="submit-btn"
                        className={`submit-btn-premium compact${isSubmitting ? " loading" : ""}`}
                      ...
                      >
                        <div className="submit-btn-content">
                          {isSubmitting ? (
                            ...
                            <>
                              <span className="btn-icon-premium">
                                <AnalyzeIcon size={16} />
                              </span>
                      ...
                      <button
                        className="clear-btn-premium compact"
                        onClick={handleClear}
                        disabled={isSubmitting || isRefining}
                      >
                        <span className="btn-icon-premium">
            ...
            <div className="assessment-steps-strip">
              <div className="assessment-steps-grid">
                <article className="assessment-step-card step-one">
                  ...
                  <p>
                    Capture spoken patient details clearly from staff or
                    attendants.
                  </p>
                </article>
                <article className="assessment-step-card step-two">
                  ...
                  <p>
                    Convert raw speech into structured symptoms, vitals, and
                    history.
                  </p>
                </article>
                <article className="assessment-step-card step-three">
                  ...
                  <p>
                    Generate fast urgency guidance for immediate emergency
                    action.
                  </p>
                </article>
              </div>
