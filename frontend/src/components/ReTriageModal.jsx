...
export default function ReTriageModal({
  patient,
  onClose,
  onRetriage,
  currentUser,
  ...
  const getSituationSummary = (oldPriority, newPriority) => {
    if (oldPriority === newPriority) {
      if (newPriority === "RED")
        return "Patient remains CRITICAL. Immediate intervention is still required.";
      if (newPriority === "YELLOW")
        return "Patient remains URGENT. Continue close monitoring and reassessment.";
      return "Patient remains STABLE. Standard observation remains appropriate.";
    }
    if (
      oldPriority === "RED" &&
      (newPriority === "YELLOW" || newPriority === "GREEN")
    ) {
    ...
    if (
      (oldPriority === "GREEN" || oldPriority === "YELLOW") &&
      newPriority === "RED"
    ) {
      ...
      <div className="retriage-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="retriage-preview-grid">
          <div className="retriage-preview-card">
            ...
            <div className="retriage-preview-row">
              <strong>Priority:</strong> {patient.priority || "GREEN"}
            </div>
            <div className="retriage-preview-row">
              <strong>Symptoms:</strong> {patient.symptoms || "Not recorded"}
            </div>
            <div className="retriage-preview-row">
          ...
          <div className={`retriage-result ${priorityClass}`}>
            ...
            <div className="retriage-priority-shift">
              <span
                style={{
                  color:
                    result.oldPriority === "RED"
                      ? "#ff4d4d"
                      : result.oldPriority === "YELLOW"
                        ? "#ffc107"
                        : "#00e676",
                }}
              >
              ...
              <span className="shift-arrow" style={{ color: "#aaa" }}>
                →
              </span>
              <span
                style={{
                  color:
                    result.newPriority === "RED"
                      ? "#ff4d4d"
                      : result.newPriority === "YELLOW"
                        ? "#ffc107"
                        : "#00e676",
                }}
              >
            ...
            <div className="retriage-result-row">
              <strong>Updated Symptoms:</strong>{" "}
              {result.updated?.symptoms || symptoms || "Not recorded"}
            </div>
            <div className="retriage-result-row">
              <strong>Updated Vitals:</strong>{" "}
              {result.updated?.vitals || vitals || "Not recorded"}
            </div>
        ...
        <div className="retriage-actions">
          <button
            className="retriage-cancel"
            onClick={onClose}
            disabled={loading || discharging}
          >
            Cancel
          </button>
          {result ? (
            <>
              ...
              {isReadyForDischarge(symptoms, vitals) ? (
                ...
                <button className="retriage-submit" onClick={onClose}>
                  Done
                </button>
              )}
              {/* ──────────────────────────────────────────────────── */}
