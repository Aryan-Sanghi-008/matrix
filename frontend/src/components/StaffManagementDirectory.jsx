  ...
  return (
    <article className="staff-assign-card">
      ...
      <div className="staff-assign-block">
        ...
        {patients.length === 0 ? (
          <div className="staff-assign-empty">
            No patients assigned right now.
          </div>
        ) : (
          <div className="staff-assign-chip-list">
            {patients.map((patient) => (
              <div className="staff-patient-chip" key={patient.id}>
                ...
                <span>
                  {formatAge(patient)} • {formatPriority(patient.priority)}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="staff-assign-block">
        ...
        {tasks.length === 0 ? (
          <div className="staff-assign-empty">
            No tasks mapped to this staff member.
          </div>
        ) : (
          <div className="staff-assign-task-list">
            {tasks.map((task) => (
              <div
                className={`staff-task-row ${task.completed ? "done" : "pending"}`}
                key={task.id}
              >
                <span className="staff-task-title">{task.title}</span>
                <span className="staff-task-meta">
...
export default function StaffManagementDirectory() {
  ...
  const nurses = useMemo(
    () => users.filter((user) => user.role === "NURSE"),
    [users],
  );

  const mapPatientsForUser = useCallback(
    (user, role) => {
  ...
  const mapTasksForUser = useCallback(
    (user) => {
      const candidateNames = [
        normalizeName(user.fullName),
        normalizeName(user.username),
      ...
      return tasks.filter((task) => {
        ...
        return candidateNames.some(
          (name) =>
            name === taskAssignee ||
            taskAssignee.includes(name) ||
            name.includes(taskAssignee),
        );
      });
    },
    [tasks],
  ...
  const unassignedTasks = useMemo(
    () => tasks.filter((task) => !getTaskAssignee(task)),
    [tasks],
  );
    ...
    <section className="staff-assignment-shell">
      <div className="staff-assignment-hero">
        <div>
          ...
          <p>
            Track which patients and tasks are assigned to doctors and nurses in
            real time.
          </p>
        </div>
        <button className="staff-assignment-refresh" onClick={loadDirectory}>
          Refresh
        </button>
      </div>
      ...
      {error ? (
        <div className="staff-assignment-state error">{error}</div>
      ) : null}
