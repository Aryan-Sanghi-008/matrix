...
export default function KanbanBoard({
  ...
  const handleToggleCollapse = useCallback((patientId) => {
    setCollapsedCards((prev) => {
      const next = {
        ...prev,
        [patientId]: !getCollapsedValue(prev, patientId),
      };
      return next;
    });
  }, []);
  ...
  return (
    <div className="board-section">
      <div className="board-header">
        <div>
          <div className="board-title-row">
            <span className="board-title-icon" aria-hidden="true">
              🏥
            </span>
            ...
            >
              <div className={`col-header ${col.headerClass}`}>
                ...
                <div className="col-info">
                  <div className={`col-title ${col.titleClass}`}>
                    {col.title}
                  </div>
                ...
                {colPatients.length === 0 ? (
                  ...
                  colPatients.map((patient) => (
                    <PatientCard
                      ...
                      patient={patient}
                      nowTs={nowTs}
                      onArchive={canArchive ? handleArchiveRequest : undefined}
                      onDismiss={removePatientFromBoard}
                      onRetriage={canRetriage ? handleRetriage : undefined}
                      onDragStart={canDragDrop ? handleDragStart : undefined}
      ...
      {deleteCandidate &&
        createPortal(
          <div
            className="staff-form-overlay"
            onClick={() => !deleting && setDeleteCandidate(null)}
          >
            <div
              className="delete-confirm-modal patient-delete-modal"
            ...
            >
              ...
              <div className="delete-confirm-body">
                <p>
                  Move{" "}
                  <strong>{deleteCandidate.name || "Unknown Patient"}</strong>{" "}
                  to the recycle bin?
                </p>
                <p className="delete-note">
                  The record stays recoverable for 10 days before permanent
                  purge.
                </p>
                <div className="patient-delete-form-field">
                  <label
                    htmlFor="patient-delete-reason"
                    className="recycle-bin-detail-label"
                  >
                    Delete Reason
                  </label>
                  <textarea
                ...
                {deleteError && (
                  <div className="form-error">⚠️ {deleteError}</div>
                )}
              </div>
              <div className="delete-confirm-actions">
                ...
                <button
                  className="user-delete-btn danger"
