  ...
  return (
    <section className="resource-allocation-shell">
      <div className="resource-allocation-hero">
        <div>
          <div className="resource-allocation-kicker">
            Operational Capacity Map
          </div>
          <h2 className="resource-allocation-title">Resource Allocation</h2>
          <p className="resource-allocation-subtitle">
            Review live zone availability, inspect occupied rooms, and expand
            capacity without leaving the triage console.
          </p>
        </div>
      ...
      {successMessage ? (
        <div className="resource-banner success">{successMessage}</div>
      ) : null}
          ...
          <form className="resource-form-card" onSubmit={handleRoomSubmit}>
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  setRoomForm({ ...roomForm, zoneId: event.target.value })
                }
                required
              >
                {zones.map((zone) => (
                  <option key={zone.id} value={zone.id}>
                    {zone.name} · {zone.priorityLevel}
                  </option>
                ))}
              </select>
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setRoomForm({ ...roomForm, roomCode: event.target.value })
                }
                placeholder="OBS-7"
                required
              />
            ...
            <button
              className="resource-submit-btn"
              type="submit"
              disabled={submittingRoom || zones.length === 0}
            >
              ...
              {zones.map((zone) => (
                <article className="resource-zone-card" key={zone.id}>
                  <div className="resource-zone-header">
                    <div>
                      <div
                        className={`resource-priority-pill ${String(zone.priorityLevel || "").toLowerCase()}`}
                      >
                        {zone.priorityLevel}
                      </div>
                      <h3>{zone.name}</h3>
                    ...
                    <div className="resource-zone-counts">
                      <strong>
                        {zone.availableRooms}/{zone.totalRooms}
                      </strong>
                  ...
                  <div className="resource-room-list">
                    {(zone.rooms || []).map((room) => (
                      <div
                        className={`resource-room-chip ${room.occupied ? "occupied" : "available"}`}
                        key={room.id}
                      >
                        <div>
                        ...
                        <div className="resource-room-status">
                          ...
                          <small>
                            {room.currentPatientName || "Open for assignment"}
                          </small>
                        </div>
                      </div>
                    ))}
