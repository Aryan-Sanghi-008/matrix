...
export default function AssignedTasksTab({ user }) {
    ...
    const unifiedWorklist = useMemo(() => {
        ...
        return [...taskItems, ...patientItems].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    }, [myTasks, patients]);
        ...
        <section className="assigned-task-shell">
            <div className="assigned-task-hero">
                <div>
                    ...
                    <p>Showing tasks and patients assigned to {user?.fullName || user?.username || 'you'}.</p>
                </div>
                <button className="assigned-task-refresh" onClick={loadData}>Refresh</button>
            </div>

            {loading && unifiedWorklist.length === 0 ? <div className="assigned-task-state">Loading your work...</div> : null}
            {error ? <div className="assigned-task-state error">{error}</div> : null}

            {!error && (
                <>
                    <div className="assigned-task-stats">
                        <div><strong>{unifiedWorklist.length}</strong><span>Total Work Items</span></div>
                        <div><strong>{pendingCount}</strong><span>Pending Actions</span></div>
                    </div>

                    {unifiedWorklist.length === 0 ? (
                        <div className="assigned-task-state">No work items are assigned to you yet.</div>
                    ) : (
                        <div className="assigned-task-list">
                            {unifiedWorklist.map(item => (
                                <div key={`${item.type}-${item.id}`} className={`assigned-task-item ${item.completed ? 'done' : 'pending'} ${item.type.toLowerCase()}`}>
                                    <div className="assigned-task-content">
                                        <div className="assigned-task-header-row">
                                            <span className={`work-type-badge ${item.type.toLowerCase()}`}>
                                        ...
                                        {item.details && <div className="assigned-task-details">{item.details}</div>}
                                        <div className="assigned-task-meta">Assigned: {formatTime(item.createdAt)}</div>
                                    </div>
                                    
                                    {item.type === 'TASK' && !item.completed && (
                                        <button className="assigned-task-complete" onClick={() => markDone(item.id)}>Mark Done</button>
                                    )}
                                    {item.type === 'TASK' && item.completed && (
                                        <span className="assigned-task-completed-label">Completed</span>
                                    )}
                                    {item.type === 'PATIENT_ASSIGNMENT' && (
                                        <span className="assigned-task-active-badge">Active Care</span>
                                    )}
                                </div>
                            ))}
                        </div>
