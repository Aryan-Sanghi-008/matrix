...
function parseHeartRate(text) {
  const m = text.match(/(?:heart rate|hr|pulse)\s*[:=]?\s*(\d{2,3})/i);
  return m ? Number(m[1]) : null;
}
function parseSpo2(text) {
  const m = text.match(
    /(?:spo2|oxygen(?:\s+saturation)?)\s*[:=]?\s*(\d{2,3})\s*%?/i,
  );
  return m ? Number(m[1]) : null;
}
...
function buildTriageExplanation(patient) {
  const combined =
    `${patient.symptoms || ""} ${patient.vitals || ""} ${patient.rawInput || ""}`.toLowerCase();
  const reasons = [];
  if (
    /chest pain|shortness of breath|breathing difficulty|unconscious|seizure|stroke|heavy bleeding|fainting/.test(
      combined,
    )
  )
    reasons.push(
      "High-risk emergency symptoms detected from intake narrative.",
    );
  const bp = parseBp(combined);
  if (bp) {
    if (bp.systolic < 90 || bp.diastolic < 60)
      reasons.push(
        `Low blood pressure (${bp.systolic}/${bp.diastolic}) suggests hemodynamic instability.`,
      );
    else if (bp.systolic > 180 || bp.diastolic > 120)
      reasons.push(
        `Severely elevated blood pressure (${bp.systolic}/${bp.diastolic}) indicates urgent cardiovascular risk.`,
      );
  }
  const hr = parseHeartRate(combined);
  if (hr && (hr > 120 || hr < 45))
    reasons.push(
      `Heart rate outside normal range (${hr} bpm) increased urgency.`,
    );
  const spo2 = parseSpo2(combined);
  if (spo2 && spo2 < 92)
    reasons.push(`Low oxygen saturation (${spo2}%) raised triage severity.`);
  if (!reasons.length) {
    if (patient.priority === "RED")
      reasons.push(
        "Priority elevated due to critical phrasing and risk markers.",
      );
    else if (patient.priority === "YELLOW")
      reasons.push("Moderate-risk signs detected; requires close monitoring.");
    else
      reasons.push(
        "No immediate high-risk markers found; case appears stable.",
      );
  }
  if (!patient.vitals || patient.vitals === "Not recorded")
    reasons.push("Vitals incomplete; confirm BP, HR, SpO2.");
  const hasVitals = !!patient.vitals && patient.vitals !== "Not recorded";
  let confidence =
    0.56 +
    (hasVitals ? 0.15 : 0) +
    (patient.symptoms ? 0.1 : 0) +
    Math.min(reasons.length, 3) * 0.06;
  if (
    patient.priority === "RED" &&
    /chest pain|unconscious|stroke|seizure/.test(combined)
  )
    confidence += 0.05;
  return {
    reasons: reasons.slice(0, 3),
    confidence: clamp(confidence, 0.55, 0.96),
    needsReview: !hasVitals || confidence < 0.72,
  };
}

function getSlaStatus(patient, nowTs) {
  const intake = patient.timestamp
    ? new Date(patient.timestamp).getTime()
    : nowTs;
  const remaining =
    (SLA_MS[patient.priority] || SLA_MS.GREEN) - Math.max(0, nowTs - intake);
  const breach = remaining < 0;
  return {
    breach,
    warning:
      !breach && remaining < (SLA_MS[patient.priority] || SLA_MS.GREEN) * 0.2,
    label: breach
  ...
  return (
    ...
    >
      <div className="card-header">
        <div className="card-header-left">
          ...
          <div className="card-meta-info">
            {patient.age && (
              <span className="card-age">Age: {patient.age}</span>
            )}
            <span className="card-id">ID: {patient.id}</span>
          </div>
          {(patient.assignedCareZone ||
            patient.assignedRoom ||
            patient.assignedNurseName) && (
            <div className="card-resource-strip">
              <span className="card-resource-chip">
                Zone: {patient.assignedCareZone || "Pending"}
              </span>
              <span className="card-resource-chip">
                Room: {patient.assignedRoom || "Pending"}
              </span>
              <span className="card-resource-chip">
                Nurse: {patient.assignedNurseName || "Pending"}
              </span>
            </div>
          )}
        </div>
        <div className="card-header-actions">
          ...
          {/* Dismiss button — only shown if user has archive access */}
          {onArchive && (
            <button
      ...
      {!collapsed && (
        <>
          <div
            className={`sla-chip${sla.breach ? " breached" : sla.warning ? " warning" : ""}`}
          >
            <span className="sla-dot"></span>
            <span className="sla-label">SLA</span>
            <span className="sla-time">{sla.label}</span>
          </div>
          ...
          {patient.vitals && patient.vitals !== "Not recorded" && (
            <div className="card-field">
              ...
              <div className="card-field-value vitals-display">
                {patient.vitals}
              </div>
            </div>
          )}
          ...
          {(patient.assignedRoom ||
            ...
            <div className="card-field">
              ...
              <div className="card-field-value">
                <div>
                  <strong>Zone:</strong> {patient.assignedCareZone || "Pending"}
                </div>
                <div>
                  <strong>Room:</strong> {patient.assignedRoom || "Pending"}
                </div>
                <div>
                ...
                <div>
                  <strong>Nurse:</strong>{" "}
                  {patient.assignedNurseName || "Pending"}
                </div>
                <div>
                  <strong>Support:</strong>{" "}
                  {patient.assignedSupportStaff || "General support"}
                </div>
                {Array.isArray(patient.assignedEquipment) &&
                  patient.assignedEquipment.length > 0 && (
                    <div>
          ...
          <div className="card-explain">
            ...
            <ul className="card-explain-list">
              {explanation.reasons.map((reason, idx) => (
                <li key={`${patient.id}-reason-${idx}`}>{reason}</li>
              ))}
            </ul>
            ...
            {explanation.needsReview && (
              <div className="review-flag">Manual review recommended</div>
            )}
          </div>
          ...
          <div className="card-workflow-actions">
            {!actionType ? (
              <div className="workflow-btn-row">
                <button
                  className="workflow-btn qr-code-btn"
                ...
                <button
                  className="workflow-btn discharge-btn"
                  onClick={() => setActionType("discharge")}
                >
                  🏠 Discharge
                </button>
                <button
                  className="workflow-btn handoff-btn"
              ...
              <div className="workflow-form">
                ...
                <div className="workflow-form-actions">
                  ...
                  <button
                    className="workflow-confirm discharge"
                    onClick={handleDischarge}
                    disabled={discharging || !actionNotes.trim()}
                  >
              ...
              <div className="workflow-form">
                ...
                <div className="workflow-form-actions">
                  ...
                  <button
                    className="workflow-confirm handoff"
