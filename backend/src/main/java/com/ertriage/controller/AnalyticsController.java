...
public class AnalyticsController {
    ...
    public AnalyticsController(PatientRepository patientRepository, PatientEventRepository eventRepository,
            PatientService patientService, UserRepository userRepository, JwtUtil jwtUtil) {
    ...
    public ResponseEntity<Map<String, Object>> getAnalytics(
        ...
        // Determine which patients this user can see
        ...
        if (currentUser == null
            ...
            // DOCTOR / NURSE — scope analytics to their assigned patients
            allPatients = patientService.filterPatientsByRole(patientRepository.findAll(), currentUser);
        }

        // Determine which events are visible (scoped to visible patient IDs)
        Set<String> visiblePatientIds = allPatients.stream()
                .map(Patient::getId)
                .collect(Collectors.toSet());

        List<PatientEvent> allEvents;
        if (currentUser == null
                || currentUser.getRole() == User.Role.ADMIN
                || currentUser.getRole() == User.Role.SUPERVISOR
                || currentUser.getRole() == User.Role.RECEPTIONIST) {
            ...
            allEvents = eventRepository.findAll().stream()
                    .filter(e -> visiblePatientIds.contains(e.getPatientId()))
