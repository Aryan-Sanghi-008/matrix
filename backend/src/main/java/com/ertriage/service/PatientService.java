...
public class PatientService {
    ...
    public List<PatientDTO> searchPatientsForUser(String query, User user) {
        if (query == null || query.trim().isEmpty()) return getPatientsForUser(user);
        List<Patient> patients = patientRepository
                .findByNameContainingIgnoreCaseOrSymptomsContainingIgnoreCase(query.trim(), query.trim());
        if (user != null) {
            patients = filterPatientsByRole(patients, user);
        }
        return patients.stream().map(this::toDTO).collect(Collectors.toList());
    }

    /**
     * Filters a list of patients based on the user's role.
     * Matches on both username (new records) and fullName (pre-migration records).
     */
